...
import {
  ...
  ChevronLeft, ChevronRight, Eye, Film, CheckCircle2, AlertCircle, Youtube, User, Bell, Clock
} from 'lucide-react';
...
export function PastorDashboard() {
  ...
  const demanderSuppressionSelection = () => {
    confirmDialog({
      message: (
        <>
          {t('dashboard.modal_delete_multiple_desc', { count: selectionnes.length })}<br/><br/>
          <span style={{ color: '#ef4444', fontWeight: 600, backgroundColor: '#fef2f2', padding: '0.5rem 1rem', borderRadius: '8px', display: 'inline-block', fontSize: '0.9rem' }}>
            <i className="pi pi-info-circle" style={{ marginRight: '0.5rem' }}></i>
            {t('dashboard.modal_delete_warning')}
          </span>
        </>
      ),
      header: t('dashboard.modal_delete_multiple_title') || 'Confirmation de suppression',
      icon: 'pi pi-exclamation-triangle',
      defaultFocus: 'reject',
      acceptClassName: 'p-button-danger',
      rejectClassName: 'p-button-text',
      acceptLabel: t('dashboard.confirm') || 'Confirmer',
      rejectLabel: t('dashboard.cancel') || 'Annuler',
      accept: async () => {
        try {
  ...
  const demanderSuppression = (predication) => {
    confirmDialog({
      message: (
        <>
          {t('dashboard.modal_delete_desc')} <strong>{predication.titre}</strong> ?<br/><br/>
          <span style={{ color: '#ef4444', fontWeight: 600, backgroundColor: '#fef2f2', padding: '0.5rem 1rem', borderRadius: '8px', display: 'inline-block', fontSize: '0.9rem' }}>
            <i className="pi pi-info-circle" style={{ marginRight: '0.5rem' }}></i>
            {t('dashboard.modal_delete_warning')}
          </span>
      ...
      header: t('dashboard.modal_delete_title') || 'Confirmation de suppression',
      icon: 'pi pi-exclamation-triangle',
      defaultFocus: 'reject',
      acceptClassName: 'p-button-danger',
      rejectClassName: 'p-button-text',
    ...
    <div className="dashboard-container">
      ...
      <aside className="dashboard-sidebar">
        <div className="sidebar-header">
          <h2>{t('dashboard.sidebar_title')}</h2>
        </div>
        ...
        <nav className="sidebar-menu">
          ...
          >
            <Film size={18} />
            <span>{t('dashboard.sidebar_videos')}</span>
          </button>
          ...
          {enEdition && (
            ...
            >
              <PencilLine size={18} />
              <span>{t('dashboard.sidebar_edit')}</span>
            </button>
          )}
          ...
          >
            <TrendingUp size={18} />
            <span>{t('dashboard.sidebar_overview')}</span>
          </button>
        </nav>
      </aside>
      ...
      <main className="dashboard-content">
        <div className="dashboard-topbar">
          <div className="dashboard-title-area">
            <p style={{ margin: 0, fontWeight: 600, color: 'var(--pd-text-muted)' }}>{t('dashboard.welcome_back', { name: pasteur?.nom_affichage || t('profile.default_pastor') })}</p>
          </div>
          <div className="topbar-actions" style={{ position: 'relative' }}>
            <button className="btn btn-outline" style={{ padding: '0.4rem', borderRadius: '50%', position: 'relative' }} onClick={() => setAfficherNotifications(!afficherNotifications)}>
              <Bell size={18} />
              {unreadCount > 0 && (
            ...
            <div className="dashboard-kpis">
              <div className="dashboard-kpi">
                ...
                <div className="kpi-content">
                  <p>{t('dashboard.online_messages')}</p>
              ...
              <div className="dashboard-kpi">
                ...
                <div className="kpi-content">
                  <p>{t('dashboard.listeners_reads')}</p>
            ...
            <div className="dashboard-section" style={{ marginTop: '1.5rem' }}>
              <div className="table-actions-top" style={{ marginBottom: '1.5rem' }}>
                <div className="left-actions">
                  <h2 style={{ margin: 0, color: 'var(--btn-dark)', fontSize: '1.25rem' }}>{t('dashboard.top_performances')}</h2>
                </div>
              </div>
                    ...
                    <div key={predication.id} className="premium-list-item">
                      <div className="item-meta-left">
                        ...
                        <div>
                          ...
                          <span className="item-subtitle">{predication.type_media === 'VIDEO' ? t('dashboard.video') : t('dashboard.audio')}</span>
                        </div>
                      </div>
