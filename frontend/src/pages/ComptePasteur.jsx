      ...
      <div className="auth-form-side">
        <div className={`auth-form-container ${mode === 'register' ? 'auth-form-container-wide' : ''}`}>
          <div className="auth-header">
            ...
            <div>
              <h1>{mode === 'login' ? t('auth.login_title') : t('auth.pastor_title')}</h1>
          ...
          <form className={`auth-form ${mode === 'register' ? 'auth-form-grid' : ''}`} onSubmit={handleSubmit}>
            ...
            <Button 
              ...
              type="submit" 
              disabled={soumission} 
              className={`auth-submit-btn ${mode === 'register' ? 'auth-field-full' : ''}`}
            >
              {soumission 
                ? (mode === 'login' ? t('auth.btn_login_loading') : t('auth.btn_register_loading')) 
                : (mode === 'login' ? t('auth.btn_login') : t('auth.btn_create_pastor'))}
            </Button>
          </form>
